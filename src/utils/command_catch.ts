...
export async function commandCatch(state: State, ...args: string[]) {
	if (args.length !== 1) {
		console.log(
			chalk.redBright("❌ You must specify one pokemon to catch. Try again, Trainer!")
		);
		return;
	}
	...
	if (pokemonData) {
		console.log(
			chalk.cyanBright(
				`You throw a Pokéball at ${chalk.yellowBright.bold(pokemonName)}... 🎯`
			)
		);
		...
		if (randomNumber <= catchNumber) {
			...
			console.log(
				chalk.red(`Oh no! ${chalk.bold(pokemonName)} escaped... Try again! 💨`)
			);
		}
	}
}
