...
export async function commandInspect(state, ...args) {
    if (args.length !== 1) {
        console.log(chalk.redBright("❌ You must specify one pokemon to inspect. Try again, Trainer!"));
        return;
    }
    const pokemonName = args[0];
    const pokemon = state.pokedex[pokemonName];
    if (!pokemon) {
        console.log(chalk.red("You haven’t caught that Pokémon yet. Try using the 'catch' command! 🧢"));
        return;
    }
    ...
    for (const stat of pokemon.stats) {
        console.log(` - ${chalk.green(stat.stat.name)}: ${chalk.whiteBright(stat.base_stat)}`);
    }
